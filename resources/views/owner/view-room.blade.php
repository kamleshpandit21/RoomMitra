...
@section('content')
    ...
    <div class="container-fluid px-4 py-5">
                    ...
                    <div class="col-md-4">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div>
                                ...
                                <span
                                    class="badge badge-{{ $room->status === 'available' ? 'success' : ($room->status === 'booked' ? 'danger' : ($room->status === 'pending' ? 'warning' : 'secondary')) }}">
                                    {{ ucfirst($room->status) }}
                                </span>
                            </div>
                        </div>
            ...
            <div class="card-body">
                <div class="row g-4">
                    @foreach ($room->images as $image)
                        <div class="col-md-4">
                            <div class="card">
                                <img src="{{ asset($image->image_url) }}" class="card-img-top img-thumbnail" alt="Room Image"
                                    data-toggle="modal" data-target="#imageModal{{ $loop->index + 1 }}">
                                <div class="card-body">
        ...
        <!-- Image Modals -->
        @foreach ($room->images as $image)
            <div class="modal fade" id="imageModal{{ $loop->index + 1 }}" tabindex="-1"
                aria-labelledby="imageModalLabel{{ $loop->index + 1 }}" aria-hidden="true">
                <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <img src="{{ asset("storage/".$image->image_url) }}" alt="Room Image" class="img-fluid">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="d-flex justify-content-between mt-4">
            ...
            <form action="{{ route('owner.rooms.destroy', $room->room_id) }}" method="POST"
                onsubmit="return confirm('Are you sure you want to delete this room?');" class="d-inline">
                @csrf
                @method('DELETE')
